<script lang="ts">
	...
	import { storageBackupPath, defaultMinifyJSON, defaultCompress } from '../../scripts/settings.ts';
	...
	function updateFileExtension(): void {
		if (filePath.endsWith('.json') || filePath.endsWith('.json.gz')) {
			if (compress && filePath.endsWith('.json')) filePath = filePath + '.gz';
			else if (!compress && filePath.endsWith('.json.gz')) filePath = filePath.slice(0, -3);
		}
	}
	...
	async function doSave(): Promise<void> {
		...
		errorMessage = '';
		try {
			const result = await api.settings.exportToFile(filePath.trim(), minifyJSONState, compress);
			if (result.success) {
