...
/**
 * Serialise request params for the log: secret values withheld, the whole blob
 * truncated. Some methods carry a whole file chunk, and a multi-megabyte log line
 * per call is both unreadable and a measurable write cost — the truncation alone
 * would not help, because the megabytes are spent building the string before it is
 * cut. A binary payload is always a plain `Uint8Array` (see
 * {@link decodeBinaryRequest}), never a `Buffer`, whose `toJSON` would run before
 * this replacer ever sees it. A redacted key is kept so the call stays
 * recognisable; only its value is withheld.
 */
export function formatParamsForLog(params: unknown): string {
	const json =
		JSON.stringify(params, (key, value) => {
			if (SECRET_PARAM_KEYS.has(key) && typeof value === 'string') return '<redacted>';
