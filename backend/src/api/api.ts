 ...
 *
 * Secret values are withheld by key, never by the type of what they hold:
 * restricting the redaction to strings meant a client could put its
 * passphrase in an array or an object and have the whole thing logged
 * verbatim — a request that is malformed for the handler is not malformed
 * for the logger, which runs first. The key itself is kept so the call
 * stays recognisable.
 */
export function formatParamsForLog(params: unknown): string {
	const json = JSON.stringify(params, (key, value) => (SECRET_PARAM_KEYS.has(key) ? '<redacted>' : value instanceof Uint8Array ? `<${value.byteLength} bytes>` : value)) ?? String(params);
