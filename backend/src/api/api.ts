 ...
 */
export function fanOutEvent(clients: Iterable<BroadcastTarget>, event: string, msg: string, except?: BroadcastTarget): number {
	let sent = 0;
...
export class APIServer {
	...
	private broadcast(event: string, data: any, except?: ClientSocket): void {
		...
		const sent = fanOutEvent(this.clients, event, msg, except);
