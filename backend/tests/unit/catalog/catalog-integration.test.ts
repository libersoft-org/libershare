...
import { initCatalogTables, getCatalogEntry, listCatalogEntries, isTombstoned, getCatalogACL, ensureCatalogACL, searchCatalog, getDeltaEntries, getVectorClock } from '../../../src/db/catalog.ts';
...
describe('E2E: Full lifecycle — publish, update, remove', () => {
	test('owner sets up ACL, moderator publishes, updates, and removes', async () => {
		...
		const { op: grantAdmin, updatedClock: c1 } = await signCatalogOp(
			ownerKey,
			'acl_grant',
			'net1',
			{
				role: 'admin',
				delegatee: admin1Key.publicKey.toString(),
			},
			ownerClock
		);
		ownerClock = c1;
		...
		const { op: grantMod, updatedClock: c2 } = await signCatalogOp(
			admin1Key,
			'acl_grant',
			'net1',
			{
				role: 'moderator',
				delegatee: mod1Key.publicKey.toString(),
			},
			adminClock
		);
		adminClock = c2;
		...
		let modClock = clock();
		const { op: addOp, updatedClock: c3 } = await signCatalogOp(
			mod1Key,
			'add',
			'net1',
			{
		...
		const { op: updateOp, updatedClock: c4 } = await signCatalogOp(
			mod1Key,
			'update',
			'net1',
			{
...
describe('E2E: Multi-peer catalog with search', () => {
	test('multiple moderators publish, search works across entries', async () => {
		...
		let ownerClock = clock();
		const { op: g1, updatedClock: oc1 } = await signCatalogOp(
			ownerKey,
			'acl_grant',
			'net1',
			{
		...
		const { op: g2 } = await signCatalogOp(
			...
			{
				role: 'moderator',
				delegatee: mod2Key.publicKey.toString(),
			},
			ownerClock
		);
		...
		for (const [id, name, desc, tags] of [
			...
			const { op, updatedClock } = await signCatalogOp(
				...
				'net1',
				{
					...
					description: desc,
					tags: [...tags],
					publisherPeerID: mod2Key.publicKey.toString(),
					publishedAt: new Date().toISOString(),
					chunkSize: 1024,
					checksumAlgo: 'sha256',
					totalSize: 2000,
...
describe('E2E: Concurrent updates — LWW resolution', () => {
	test('two moderators update same entry — higher HLC wins', async () => {
		...
		// Mod1 adds entry
		let m1Clock = clock();
		const { op: addOp, updatedClock: m1c2 } = await signCatalogOp(
			mod1Key,
			'add',
			'net1',
			{
		...
		const { op: updateA } = await signCatalogOp(
			mod1Key,
			'update',
			'net1',
			{
		...
		const { op: updateB } = await signCatalogOp(
			mod2Key,
			'update',
			'net1',
			{
...
describe('E2E: Security — unauthorized actions blocked', () => {
	...
	test('moderator cannot grant admin role', async () => {
		...
		const { op: escalate } = await signCatalogOp(
			mod1Key,
			'acl_grant',
			'net1',
			{
	...
	test('revoked moderator writes are rejected', async () => {
		let ownerClock = clock();
		// Grant moderator
		const { op: grant, updatedClock: oc2 } = await signCatalogOp(
			ownerKey,
			'acl_grant',
			'net1',
			{
		...
		// Mod publishes
		let modClock = clock();
		const { op: add, updatedClock: mc2 } = await signCatalogOp(
			mod1Key,
			'add',
			'net1',
			{
		...
		const { op: revoke } = await signCatalogOp(
			ownerKey,
			'acl_revoke',
			'net1',
			{
...
describe('E2E: Multi-network isolation', () => {
	test('entries in different networks are isolated', async () => {
		...
		const { op: g1 } = await signCatalogOp(
			ownerKey,
			'acl_grant',
			'net1',
			{
		...
		const { op: a1 } = await signCatalogOp(
			mod1Key,
			'add',
			'net1',
			{
		...
		const { op: a2 } = await signCatalogOp(
			mod2Key,
			'add',
			'net2',
			{
		...
		const { op: cross } = await signCatalogOp(
			mod1Key,
			'add',
			'net2',
			{
