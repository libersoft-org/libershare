		...
		for (let m = 0; m < 10; m++) {
			...
			for (let i = 0; i < 10; i++) {
				const { op, updatedClock } = await signCatalogOp(
					...
					'net1',
					{
						lishID: `mod${m}-entry${i}`,
						name: `Mod${m} Entry${i}`,
						publisherPeerID: mods[m]!.publicKey.toString(),
						publishedAt: new Date().toISOString(),
						chunkSize: 1024,
						checksumAlgo: 'sha256',
						totalSize: 500,
...
describe('Edge case: Tombstone interactions', () => {
	test('remove then newer re-add resurrects the entry (LWW-element-set)', async () => {
		...
		let modClock = clock();
		// Add
		const { op: add1, updatedClock: c1 } = await signCatalogOp(
			mod,
			'add',
			'net1',
			{
...
describe('Edge case: Field size limits at boundary', () => {
	...
	test('exactly 10 tags passes, 11 tags fails', async () => {
		...
		let modClock = clock();
		const { op: op10, updatedClock } = await signCatalogOp(
			mod,
			'add',
			'net1',
			{
		...
		const { op: op11 } = await signCatalogOp(
			mod,
			'add',
			'net1',
			{
...
describe('Edge case: ACL cascading revocation', () => {
	test('revoking admin removes them but moderators stay (simplified model)', async () => {
		...
		// Owner grants admin
		const { op: ga, updatedClock: c1 } = await signCatalogOp(
			ownerKey,
			'acl_grant',
			'net1',
			{
				role: 'admin',
				delegatee: admin.publicKey.toString(),
			},
			oClock
		);
		oClock = c1;
		...
		const { op: gm } = await signCatalogOp(
			admin,
			'acl_grant',
			'net1',
			{
		...
		// Owner revokes admin
		const { op: ra } = await signCatalogOp(
			ownerKey,
			'acl_revoke',
			'net1',
			{
