...
import { COMPRESSION_ALGORITHMS, ErrorCodes, MAX_UPLOAD_CHUNK_SIZE, compressionExtension, detectCompression } from '@shared';
...
describe('decodeBinaryRequest', () => {
	...
	it('rejects a header that is not a request object', () => {
		for (const header of [null, 1, 'request', []]) expect(() => decodeBinaryRequest(frame(header as any, new Uint8Array()))).toThrow(ErrorCodes.PARSE_ERROR);
	});

	it('rejects non-object params while preserving a usable request id', () => {
		try {
			decodeBinaryRequest(frame({ id: 'bad-params', method: 'upload.chunk', params: 'not-an-object' }, new Uint8Array()));
			throw new Error('expected the frame to be rejected');
		} catch (err) {
			expect(err).toBeInstanceOf(BinaryFrameError);
			expect((err as BinaryFrameError).requestID).toBe('bad-params');
		}
	});
});
...
describe('upload sweep', () => {
	...
	it('keeps charging for an upload whose file could not be deleted', async () => {
		...
		// The abort does not get to report success over a file it failed to remove:
		// the caller's next upload starts on the strength of that answer, and the
		// quota it would be starting against is still spoken for.
		await expect(handlers.abort({ uploadID }, client)).rejects.toThrow(ErrorCodes.UPLOAD_CLEANUP_PENDING);
		// Unreachable for its owner all the same — the abort did happen.
