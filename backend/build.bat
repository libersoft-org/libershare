...
for /f "tokens=*" %%n in ('node -p "require('../shared/src/product.json').name"') do set "PRODUCT_NAME=%%n"
for /f "tokens=*" %%v in ('node -p "require('../shared/src/product.json').version"') do set "PRODUCT_VERSION=%%v"
