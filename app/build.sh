...
sync_product_info() {
	...
	if [ "$_PRODUCT_INFO_SYNCED" != "1" ]; then
		# tauri.conf.json + tauri.linux.conf.json are generated (and gitignored)
		# from their *.template files via shared/src/product.json - no .orig backup
		# needed. Cargo.toml is still patched in place.
		bun "$SCRIPT_DIR/sync-config.ts"
		# Only backup when NOT running as docker-inner; the outer script
		# (build.bat or build.sh orchestrator) owns the .orig lifecycle.
		if [ "$DOCKER_INNER" != "1" ]; then
	...
	if [ "$BUILD_OS" = "linux" ] && [ "$_SYNCED_LINUX" != "1" ]; then
		# tauri.linux.conf.json is generated by sync-config.ts above; only the
		# desktop-entry files are patched in place here.
		if [ "$DOCKER_INNER" != "1" ]; then
...
for _bkp in Cargo.toml wix-fragment-debug.wxs desktop-entry-debug.desktop desktop-entry.desktop; do
