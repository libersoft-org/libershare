...
cleanup() {
	...
	[ -n "$LDD_WRAPPER_DIR" ] && rm -r "$LDD_WRAPPER_DIR" 2>/dev/null
...
# ── Clean old output ──
[ -d "$SCRIPT_DIR/build/release/bundle" ] && rm -r "$SCRIPT_DIR/build/release/bundle"
[ -d "$SCRIPT_DIR/icons" ] && rm -r "$SCRIPT_DIR/icons"
[ -d "$ROOT_DIR/frontend/build" ] && rm -r "$ROOT_DIR/frontend/build"
